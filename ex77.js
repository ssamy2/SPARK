...
document.addEventListener('DOMContentLoaded', () => {
    ...
    button.addEventListener('click', () => {
        AdController.show().then((result) => {
            ...
            showNotificationWithStatus(purchaseNotification, `You got me ${rewardAmount} $SPARK for watching the ad`, 'win');
        }).catch((result) => {
    ...
    fetch('json/tasks.json')
        ...
        .then(tasks => {
            tasks.forEach(task => {
                ...
                rewardText.textContent = `+ ${task.reward} $SPARK`;
...
async function claimTaskReward(taskId, reward, button, completedTasks) {
    try {
        ...
        button.textContent = '✓';
        button.disabled = true;
        showNotificationWithStatus(uiElements.purchaseNotification, `Successfully claimed ${reward} $SPARK!`, 'win');
...
function showBanScreen() {
    ...
    contactSupport.onclick = () => {
        window.location.href = 'https://t.me/SPaRKaIRoBot'; 
    };
...
const tonConnectUI = new TON_CONNECT_UI.TonConnectUI({
  ...
  uiOptions: {
    twaReturnUrl: 'https://t.me/SPaRKaIRoBot/SPARK',
  },
});
...
document.getElementById('ton').addEventListener('click', async () => {
  ...
  try {
    ...
    const recipientAddress = "UQBq52EoYK7FxD-cSlW49ENlMEE2BtjMYjN2f6vkUajUNYJn";
